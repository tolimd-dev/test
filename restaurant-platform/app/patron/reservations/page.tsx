...
export default async function PatronReservationsPage() {
  ...
  const upcoming = reservations.filter(r => new Date(r.session.date) >= new Date())
    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      {reservations.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-xl border border-stone-200">
          <p className="text-gray-400 text-lg mb-4">You haven&apos;t reserved any experiences yet.</p>
          <Link
            href="/"
...
function ReservationCard({ reservation }: {
  ...
  const totalCost = session.event.basePrice + session.event.pricePerPerson * reservation.guests
  const perPerson = (totalCost / reservation.guests).toFixed(2)
  const myTotal = (parseFloat(perPerson) * reservation.guests).toFixed(2)
  ...
  return (
    ...
    >
      <div>
        <h3 className="font-bold text-gray-900 mb-1">{session.event.title}</h3>
        <p className="text-sm text-gray-500">
          {date.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}
          {" at "}
          {date.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}
        </p>
        <p className="text-xs text-gray-400 mt-1">Hosted by {session.event.host.name}</p>
      </div>
      <div className="text-right">
        <p className="text-amber-700 font-bold">${myTotal}</p>
